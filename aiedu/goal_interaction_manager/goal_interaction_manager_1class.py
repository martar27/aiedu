...
class GoalInteractionManager:
    def __init__(self):
        self.db_manager = DatabaseManager(host='localhost', database='eduai1', user='mysql_admin', password='Mysql#2869')
        self.api_client = APIClient()
        self.interaction_counts = {}
        self.interaction_threshold = 3

    # Goal Manager Methods
    ...
    def run(self):
        ...
        if user_id:
            ...
            self.define_goal(user_id, session_id)
            self.display_final_goal(user_id, session_id)
            self.initiate_dialogue(user_id)
        else:
            print("No user found in the database.")

if __name__ == "__main__":
    manager = GoalInteractionManager()
    manager.run()